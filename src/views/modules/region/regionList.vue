<template>
    <div>
        ...
        <el-col :span="24" v-loading="loading"  element-loading-text="正在查询中。。。" >
          <el-table ref="multipleTable" :data="regionData" tooltip-effect="dark" style="width: 100%" @selection-change="handleSelectionChange">
    ...
    methods: {
      ...
      handleDelete(index,row){
        ...
        this.$confirm('此操作将永久删除该条数据, 是否继续?', '提示', {confirmButtonText: '确定',cancelButtonText: '取消', type: 'warning'
            ...
            API.DeleteRegion({id:row.id}).then((res) => {
              if(res.code == 0){
                that.GetLabelList();
